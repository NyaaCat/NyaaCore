...
public class SQLiteProvider implements DatabaseProvider {
    ...
    public SQLiteDatabase get(Plugin plugin, Map<String, Object> configuration) {
        ...
        if(Boolean.parseBoolean(configuration.get("autoscan").toString())){
            ...
            try {
                classes = ClassPath.from(((JavaPlugin) plugin).getClass().getClassLoader()).getAllClasses()
                                   .stream()
                                   .filter(c -> pack == null || c.getPackageName().startsWith((String) pack))
                                   .map(ClassPath.ClassInfo::load)
                                   .filter(c -> c.getAnnotation(DataTable.class) != null)
                                   .toArray(Class<?>[]::new);
            } catch (IOException e) {
...
final class ClassPath {
    ...
    abstract static class Scanner {
        ...
        final void scan(ClassLoader classloader) throws IOException {
